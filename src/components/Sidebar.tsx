...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div
        className={`flex items-center h-16 border-b border-border px-4 gap-3 flex-shrink-0 ${
        ...
        {groups.map((group) => {
          ...
          return (
            <div key={`group-${group.key}`} className="mb-4">
              {!collapsed && (
                <p className="text-xs font-600 text-muted-foreground uppercase tracking-wider px-2 mb-1">
                  {group.label}
                </p>
              )}
              {collapsed && (
                <div className="border-t border-border mb-2 mx-1" />
              )}
              {groupItems.map((item) => {
                const isActive =
                  item.href === '/'
                ...
                return (
                  ...
                  >
                    ...
                    {!collapsed && item.badge !== undefined && (
                      <span className="ml-auto bg-primary text-primary-foreground text-xs font-600 rounded-full px-1.5 py-0.5 leading-none tabular-nums">
                        {item.badge}
                      </span>
                    )}
                    {collapsed && item.badge !== undefined && (
      ...
      <div className="border-t border-border p-2 flex-shrink-0">
        <button
          className={`flex items-center gap-3 px-2 py-2 rounded-lg text-sm text-muted-foreground hover:bg-muted hover:text-foreground w-full transition-all duration-150 ${
            collapsed ? 'justify-center' : ''
          }`}
        ...
        <div
          className={`flex items-center gap-2.5 px-2 py-2 mt-1 ${
            collapsed ? 'justify-center' : ''
          }`}
        >
