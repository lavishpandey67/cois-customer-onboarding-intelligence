...
const fromCookies = () =>
  ...
  document.cookie.split(';').filter(Boolean).map((c) => {
    ...
    const cookieName = eqIdx >= 0 ? c.trim().slice(0, eqIdx) : c.trim();
    const cookieValue = eqIdx >= 0 ? decodeURIComponent(c.trim().slice(eqIdx + 1)) : '';
    return { name: cookieName.trim(), value: cookieValue };
  }).filter((c) => c.name);
